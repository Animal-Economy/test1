...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Tajcoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Tajcoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Tajcoin is not connected!");

    //if (IsInitialBlockDownload())
    //    throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Tajcoin is downloading blocks...");
