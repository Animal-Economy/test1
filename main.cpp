...
const string strMessageMagic = "Tajcoin Signed Message:\n";
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsTajcoinV2(nHeight) && nVersion < 7)
        return DoS(100, error("AcceptBlock() : reject too old nVersion = %d", nVersion));
    else if (!IsTajcoinV2(nHeight) && nVersion > 7)
    ...
    if(IsProtocolV0(nHeight))
          {
            if (GetBlockTime() > FutureDriftV0((int64_t)vtx[0].nTime, nHeight))
...
#ifdef ENABLE_WALLET
// Tajcoin: attempt to generate suitable proof-of-stake
