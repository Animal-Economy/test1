...
{
    // Convert Tajcoin:// to Tajcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Tajcoin://"))
    {
        uri.replace(0, 12, "Tajcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Tajcoin.lnk";
}
...
{
    return GetAutostartDir() / "Tajcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Tajcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Tajcoin-qt [" + tr("command-line options") + "]                     " + "\n";
