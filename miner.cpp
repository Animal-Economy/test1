...
CBlock* CreateNewBlock(CReserveKey& reservekey, bool fProofOfStake, int64_t* pFees)
{
    ...
    // 1-tajshi-fee transactions. It should be set above the real
...
{
    LogPrintf("TajcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Tajcoin-miner");
	...
	CBlock *pblock = pblocktemplate.get();
        ...
        LogPrintf("Running TajcoinMiner with %llu transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("TajcoinMiner terminated\n");
        throw;
    }
}
